...
from dataset.KeyFrame_AnhLe.DE_Entropy4 import evolution4
...
def extract_keyframes(source, dest, numGeneration, popSize, frame_percent=0.1, ):
    ...
    dest = os.path.join(dest, current_dirname + 'keyframes_Entropy_{:.02f}'.format(frame_percent))
    ...
    for i in range(len(videos)):
        ...
        if video_name in {'05', '11', '12', '13', '14', '15', '16'}:
            evolution(sourceEvo, dest, numGeneration, popSize, numFrames, numKeyFrames, keyframe_dir, path2file)
        else:
            print('Not in')
            evolution4(sourceEvo, dest, numGeneration, popSize, numFrames, numKeyFrames, keyframe_dir, path2file)
