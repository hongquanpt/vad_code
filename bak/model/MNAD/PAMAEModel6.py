from .PAMAEModel5 import PAMAEModel5
...
class PAMAEModel6(PAMAEModel5):
    ...
    def get_data_patch(self):
        try:
            ...
            self.iter_patch = iter(self.patch_dataloader)
            data_patch, _ = next(self.iter_patch)
        return data_patch, _
    ...
    def train_epoch(self, epoch, train_dataloader):
        ...
        self.iter_patch = iter(self.patch_dataloader)
        data_augtypes = ['keyframes', 'patch']
        for j, data in enumerate(train_dataloader):
            # data.shape = torch.Size([4, 15, 256, 256])
            # self.logger.info(f'data.shape = {data.shape}')
            is_pseudo_labels = []
            data_augtype = random.choice(data_augtypes)
            data_kf = None
            data_patch = None
            if data_augtype == 'keyframes':
            ...
            for b in range(bz):
                ...
                if is_pseudo:
                    if data_augtype == 'keyframes':
                        ...
                        data[b], mask = patch_SmoothMixS(data[b].unsqueeze(0),
                                                         data_patch.squeeze(0),
                                                         max_size=self.cfg.TRAIN.other_params.max_size,
                                                         h=self.cfg.DATASET.width,
                                                         w=self.cfg.DATASET.height,
                                                         max_move=self.cfg.TRAIN.other_params.max_move)
    ...
    def train(self, train_dataloader, checkpoint_filepath, checkpoint_dir_path):
        self.logger.info('call: train() in PAMAE6Model')
