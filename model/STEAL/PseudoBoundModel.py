...
# =======================================
class PseudoBoundModel(STEALModel):
    ...
    def train(self, train_dataloader, checkpoint_filepath, checkpoint_dir_path):
        self.logger.info('call: train() in PseudoBoundModel')
        return super().train(train_dataloader, checkpoint_filepath, checkpoint_dir_path)

    def train_epoch(self, epoch, train_dataloader):
        if self.cfg.MODEL.method == 'PseudoBound':
            return self.PseudoBound(epoch, train_dataloader)

    def PseudoBound(self, epoch, train_dataloader):
        ...
        for j, data in enumerate(train_dataloader):
            ...
            for b in range(bz):
                ...
                if is_pseudo:
                    if self.data_patch['technique'] == 'SmoothMixS':
                        ...
                        # data[b] (1,16,256,256)
                        img_patch, mask = patch_SmoothMixS(data[b],
            ...
            pixel_loss = self.loss_fn(output, data)
            modified_loss_mse = []
            for b in range(bz):
    ...
    def test(self, test_dataloader, video_labels, checkpoint_filepath, visualization_dir_path):
        self.logger.info('call: test() in PseudoBoundModel')
