...
from model.AMC.AMCModel import AMCModel
...
def train(cfg, device, logger, writer, checkpoint_dir_path):
    ...
    # ====================================
    # 4. load model
    AMC = AMCModel(cfg, device, logger, writer)
    ...
    AMC.train(train_dataloader, checkpoint_filepath, checkpoint_dir_path)
    return 1
...
def test(cfg, device, logger, writer, visualization_dir_path):
    ...
    AMC.test(train_dataloader, test_dataloader, label_list, checkpoint_filepath, sub_visualization_dir_path)
    return 0
...
if __name__ == '__main__':
    """
    python main_AMC.py --phase 'train' --dataset 'ped2' SYSTEM.device "cuda:0"
    """
    main(net='AMC')
