...
# step 1: chmod u+x system/VideoVAE/train.sh
# step 2: ./system/VideoVAE/train.sh
...
config_filepath='system/VideoVAE/ped2.yaml' # avenue.yaml, shanghaitech.yaml
#loop=(1 2 3 4 5)
loop=(1)
...
for id in "${loop[@]}"; do
    echo ">>> main_VideoVAE.py >>>>"
    python main_VideoVAE.py --cfgfile "$config_filepath" SYSTEM.phase 'train' SYSTEM.loop_index "$id"
done
conda deactivate