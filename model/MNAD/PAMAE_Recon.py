...
class Encoder_Recon(torch.nn.Module):
    def __init__(self, num_frames = 1, num_channels =3):
        super(Encoder_Recon, self).__init__()
        ...
        self.moduleConv1 = Basic(num_channels*(num_frames), 64)
...
class Decoder_Recon(torch.nn.Module):
    def __init__(self, num_frames = 1, num_channels =3):
        ...
        self.moduleDeconv1 = Gen(64,num_channels,64)
...
class PAMAE_Recon(torch.nn.Module):
    def __init__(self, num_channels =3,  num_frames = 1, memory_size = 10, feature_dim = 512, key_dim = 512, temp_update = 0.1, temp_gather=0.1, device="cuda:0"):
        super(PAMAE_Recon, self).__init__()

        self.encoder = Encoder_Recon(num_frames, num_channels)
        self.decoder = Decoder_Recon(num_frames, num_channels)
        self.memory = Memory(memory_size,feature_dim, key_dim, 
                             temp_update, temp_gather, device)
       
    def forward(self, x, keys, train=True, pseudo_anomaly = False):
        fea, skip1, skip2, skip3 = self.encoder(x)
        if train:
            updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss = self.memory(fea, keys, train, pseudo_anomaly)
