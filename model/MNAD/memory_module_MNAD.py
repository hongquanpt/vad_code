...
class Memory(nn.Module):
    ...
    def get_update_query(self, mem, max_indices, update_indices, score, query, train):
        ...
        param max_indices: Tensor(2048,1)
        param update_indices: Tensor(1,10)
        param score: Tensor(2048,10)
        param query: Tensor(2048,512)
        """
        m, d = mem.size()
        if train:
    ...
    def forward(self, query, keys, train=True):  # keys(10,512)
    ...
    def gather_loss(self, query, keys, train):  # keys(10,512)
        batch_size, h, w, dims = query.size()  # b X h X w X d (4, 32, 32, 512)
        if train:
            ...
            return gathering_loss, top1_loss  # separateness_loss, compactness_loss
        else:
            loss_mse = torch.nn.MSELoss()
            softmax_score_query, softmax_score_memory = self.get_score(keys, query)
            query_reshape = query.contiguous().view(batch_size * h * w, dims)
