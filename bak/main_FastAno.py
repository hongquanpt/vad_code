...
from dataset.process import get_train_dataset_aug_FastAno, get_test_dataset
...
def train(cfg, device, logger, writer, checkpoint_dir_path):
    ...
    train_dataset, train_dataloader = get_train_dataset_aug_FastAno(cfg)
    ...
    # ====================================
    # 5. load model
    fast_ano = FastAnoModel(cfg, device, logger, writer)
    ...
    fast_ano.train(train_dataloader, checkpoint_filepath, checkpoint_dir_path)
    return 1
...
def test(cfg, device, logger, writer, visualization_dir_path):
    ...
    fast_ano.test(test_dataset, test_dataloader, label_list, checkpoint_filepath, sub_visualization_dir_path)
    return 0
...
if __name__ == '__main__':
    """
    python main_FastAno.py --phase 'train' --dataset 'ped2' SYSTEM.device "cuda:0"
    """
    main(net='FastAno')
