...
from dataset.process import get_train_dataset, get_keyframes_dataset, get_test_dataset
from dataset.label import LabelVideoDataset
from dataset.samples import get_cifar100_dataset
from bak.model.MNAD.PAMAEModel6 import PAMAEModel6
...
def train(cfg, device, logger, writer, checkpoint_dir_path):
    ...
    # 2. dataset, dataloader (keyframes)
    train_dataset_kf, train_dataloader_kf = get_keyframes_dataset(cfg)

    logger.info(f'len(train_dataset_kf) = {len(train_dataset_kf)}')
    logger.info(f'len(train_dataloader_kf) =  len(train_dataset_kf) / batch_size = {len(train_dataloader_kf)}')

    patch_dataset, patch_dataloader = get_cifar100_dataset()
    # ====================================
    # MODEL: model, memory_items, loss_fn, optimizer, scheduler
    # ====================================
    # 4. load model
    pamae6 = PAMAEModel6(cfg, device, logger, writer)
    ...
    pamae6.train(train_dataloader, train_dataloader_kf, patch_dataloader, checkpoint_filepath, checkpoint_dir_path)
    return 1
...
def test(cfg, device, logger, writer, visualization_dir_path):
    ...
    pamae6.test(test_dataloader, label_list, checkpoint_filepath, sub_visualization_dir_path)
    return 0
...
if __name__ == '__main__':
    """
    python main_PAMAE6.py --phase 'train' --dataset 'ped2' SYSTEM.device "cuda:0"
    """
    main(net='PAMAE6')
