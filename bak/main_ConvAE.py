import argparse
from system.ConvAE.default import update_config, cfg # for ConvVAE
from system.output import * # logger, make_output_dir
import torch.backends.cudnn as cudnn

from dataset.process import *

from model.ConvAE.ConvVAEModel2 import ConvVAEModel2
from model.ConvAE.ConvAE_SVMModel import ConvAE_SVMModel
...
def train(cfg, device, logger, writer, checkpoint_dirpath):
    ...
    #====================================
    if cfg.MODEL.name == 'ConvVAE_Recon':
        ConvVAE = ConvVAEModel2(cfg, device, logger, writer)
        ConvVAE.train(train_dataloader, checkpoint_dirpath)
    elif cfg.MODEL.name == 'ConvAE_SVM':
        ConvAE_SVM = ConvAE_SVMModel(cfg, device, logger, writer)
        ConvAE_SVM.train(train_dataloader, checkpoint_dirpath)
    return 1
...
def test(cfg, device, logger, writer, visualization_dirpath):
    ...
    if cfg.MODEL.name == 'ConvVAE_Recon':
        ...
        for i in range(0, len(cfg.TEST.checkpoint_filepaths)):
            ...
            ConvVAE.test(test_dataloader, label_list, checkpoint_filepath, sub_visualization_dirpath)
    elif cfg.MODEL.name == 'ConvAE_SVM':
        ConvAE_SVM = ConvAE_SVMModel(cfg, device, logger, writer)
        for i in range(0, len(cfg.TEST.checkpoint_filepaths)):
            logger.info(f'checkpoint_filepath[{i}] = {cfg.TEST.checkpoint_filepaths[i]}')
            checkpoint_filepath = cfg.TEST.checkpoint_filepaths[i]
            ocsvm_filepath = cfg.TEST.ocsvm_filepaths[i]
            train_session_name = checkpoint_filepath.split('/')[-3]
            epoch_session_name = checkpoint_filepath.split('/')[-1].split('.')[0]
            logger.info(f'train_session_name = {train_session_name}')
            logger.info(f'epoch_session_name = {epoch_session_name}')
            sub_visualization_dirpath = make_dir_path(visualization_dirpath, 
                                                      train_session_name + '/' + epoch_session_name)
            ConvAE_SVM.test(test_dataloader, label_list, checkpoint_filepath, ocsvm_filepath, sub_visualization_dirpath)
    return 0
...
def parse_args():
    ...
    cfg_filename = 'system/ConvAE/ped2-ConvAE_SVM.yaml'
