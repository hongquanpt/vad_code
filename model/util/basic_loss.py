...
class GradientLoss(nn.Module):
    def __init__(self):
        ...
        neg = (-1 * pos) + 1 - 1
        self.alpha = 1
        self.filter_x = torch.unsqueeze(torch.stack([neg, pos], dim=0), 0).permute(2, 3, 0,
                                                                                   1).cuda()  # c_out, c_in, h, w
        self.filter_y = torch.stack([torch.unsqueeze(neg, dim=0), torch.unsqueeze(pos, dim=0)]).permute(2, 3, 0,
                                                                                                        1).cuda()
        # strides = [1,1]
...
class WeightedPredLoss(nn.Module):
    ...
    def forward(self, x, target):
        ...
        weighted_error = [torch.mean(torch.pow(x[:, :, i, :, :] - target[:, :, i, :, :], 2)) * weight[i] for i in
                          range(pred_len)]
