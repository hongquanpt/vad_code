...
def train(cfg, device, logger, writer, checkpoint_dir_path):
    ...
    train_dataset, train_dataloader = get_train_dataset(cfg, 'clips')
    ...
    # ====================================
    # 4. load model
    pamae = PAMAEModel(cfg, device, logger, writer)
    ...
    pamae.train(train_dataloader, train_dataloader_sf, checkpoint_filepath, checkpoint_dir_path)
    return 1
...
def test(cfg, device, logger, writer, visualization_dir_path):
    ...
    pamae.test(test_dataloader, label_list, checkpoint_filepath, sub_visualization_dir_path)
    return 0
...
if __name__ == '__main__':
    """
    python main_PAMAE.py --phase 'train' --dataset 'ped2' SYSTEM.device "cuda:0"
    python main_PAMAE.py --phase 'test' --dataset 'ped2' SYSTEM.device "cuda:0"
    """
    main(net='MNAD')