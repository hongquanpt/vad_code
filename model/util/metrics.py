...
def psnr_1(output, target):
    ...
    psnr = 10 * math.log10(1 / pixel_loss)
    return psnr
...
def calculate_auc_scores_MNAD(pf, video_psnr, video_feas_distance, video_labels, alpha=0.6):
    """
    Calculate AUC scores for model: MNAD, MNAD (AE with memory)
    param pf(processed_frames) = 4
    """
    scores = np.array([], dtype=np.float64)
    labels = np.array([], dtype=np.int64)
    ...
    for i in range(len(video_psnr)):  # the number of testing videos
        ...
        scores = np.concatenate((scores, score), axis=0)
        # trong mỗi video, chỉ xét từ frame có stt là pf=4 đến frame cuối cùng
        # (vì 4 frames đầu tiên được dùng để dự đoán frame thứ 5)
        labels = np.concatenate((labels, video_labels[i][pf:]), axis=0)
...
def calculate_anomaly_scores(frame_psnr):
    score_psnr = minmax_normalization(frame_psnr,
                                      np.max(frame_psnr),
                                      np.min(frame_psnr))
...
def calculate_auc_scores(pf, video_psnr, video_labels):
    """
    Calculate AUC scores for model: FastAno, STEAL
    param pf(processed_frames) = 4 (FastAno) model_type == 'prediction'
    param pf(processed_frames) = 8 (STEAL)  model_type == 'reconstruction'
    """
    scores = np.array([], dtype=np.float64)
    labels = np.array([], dtype=np.int64)

    for i in range(len(video_psnr)):  # for each testing videos
        # score is anomaly scores of frames in video i-th
        score_psnr = minmax_normalization(video_psnr[i],
                                          np.max(video_psnr[i]),
                                          np.min(video_psnr[i]))
        ...
        # trong mỗi video, xét từ frame có stt là pf=4 đến frame cuối cùng
        # (vì 4 frames đầu tiên được dùng để dự đoán frame thứ 4)
        # model_type == 'reconstruction':
        # trong mỗi video, xét từ frame có stt là pf=8 đến frame có stt là len(videos[i])-8
        labels = np.concatenate((labels, video_labels[i][pf:pf + len(score)]), axis=0)
...
# ==================================================================
# ==================================================================
def calculate_auc_scores_ConvVRNN(pf, loss_list, video_labels):
    score_list = []
    scores = np.array([], dtype=np.float)
    labels = np.array([], dtype=np.int)

    for i in range(len(loss_list)):  # the number of testing videos
        # score is anomaly scores of frames in video i-th
        score_loss = 1 - minmax_normalization(loss_list[i],
                                              np.max(loss_list[i]),
                                              np.min(loss_list[i]))
        score_list.append(score_loss)
        scores = np.concatenate((scores, score_loss), axis=0)
        labels = np.concatenate((labels, video_labels[i][pf:]),
                                axis=0)  # trong mỗi video, chỉ xét từ frame có stt là pf=4 đến frame cuối cùng (vì 4 frames đầu tiên được dùng để dự đoán frame thứ 4)
    ...
    print('labels.shape =', labels.shape)  # labels.shape = (1962,)
    # score_list: is a list of anomaly scores of each video
    return auc, fpr, tpr, score_list


def calculate_auc_scores_ConvLSTM_AE(loss_list, video_labels):
    score_list = []
    scores = np.array([], dtype=np.float)
    labels = np.array([], dtype=np.int)
    ...
    for i in range(len(loss_list)):  # the number of testing videos
        # score is anomaly scores of frames in video i-th
        score_loss = 1 - (loss_list[i] - np.min(loss_list[i])) / np.max(loss_list[i])
...
def calculate_TP_FN_FP_TN(labels, predicts):
    TP = FN = FP = TN = 0
    # labels == 1: anomaly, labels == -1: normal, 
    # predicts == 1: anomaly, predicts == -1: normal
    for j in range(len(labels)):
        if labels[j] == 1 and predicts[j] == 1:
            TP = TP + 1
        elif labels[j] == 1 and predicts[j] == -1:
            FN = FN + 1
        elif labels[j] == 0 and predicts[j] == 1:
            FP = FP + 1
        else:
            TN = TN + 1
    return TP, FN, FP, TN
...
def calculate_accuracy_ocsvm(video_scores_list, video_labels):
    predicts = np.array([], dtype=np.float)
    labels = np.array([], dtype=np.int)
    for i in range(len(video_scores_list)):  # the number of testing videos
        if video_scores_list[i] > 0:  # inlier
            predicts = np.concatenate((predicts, 1), axis=0)
        else:  # outlier
            predicts = np.concatenate((predicts, -1), axis=0)

        labels = np.concatenate((labels, video_labels[i]), axis=0)

    assert predicts.shape == labels.shape, f'Ground truth has {labels.shape[0]} frames, BUT got {predicts.shape[0]} detected frames!'

    TP, FN, FP, TN = calculate_TP_FN_FP_TN(labels, predicts)
    accuracy = (TP + TN) / (TP + FN + FP + TN)
    print(f'accuracy = {accuracy}')

    # scores: is a list of anomaly scores of all videos
    print('predicts.shape =', predicts.shape)  # scores.shape = (1962,)
    print('labels.shape =', labels.shape)  # labels.shape = (1962,)
    # score_list: is a list of anomaly scores of each video
    return accuracy
...
def anomaly_score_list(video_psnr):  # video_psnr cua 1 video
    anomaly_score_list = list()
    for i in range(len(video_psnr)):
        anomaly_score_list.append(anomaly_score(video_psnr[i], np.max(video_psnr), np.min(video_psnr)))

    return anomaly_score_list


def anomaly_score_list_inv(video_psnr):
    anomaly_score_list = list()
    for i in range(len(video_psnr)):
        anomaly_score_list.append(anomaly_score_inv(video_psnr[i], np.max(video_psnr), np.min(video_psnr)))
...
def calculate_auc_scores_MNAD_author(pf, video_psnr, feas_distance_list, video_labels, alpha=0.6):
    """
    Calculate AUC scores for model: MNAD, MNAD (AE with memory)
    """
    score_list = []
    ...
    for i in range(len(video_psnr)):  # the number of testing videos
        # score is anomaly scores of frames in video i-th
        score_psnr = anomaly_score_list(video_psnr[i])
        score_feas_distance = anomaly_score_list_inv(feas_distance_list[i])
        # score = alpha*score_psnr + (1.0 - alpha)*score_feas_distance
        score = score_sum(score_psnr, score_feas_distance, alpha)
        score_list.append(score)
        anomaly_score_total_list += score

        labels_list = np.append(labels_list, video_labels[i][
                                             pf:])  # trong mỗi video, chỉ xét từ frame có stt là pf=4 đến frame cuối cùng (vì 4 frames đầu tiên được dùng để dự đoán frame thứ 4)
    anomaly_score_total_list = np.asarray(anomaly_score_total_list)

    assert len(anomaly_score_total_list) == len(
        labels_list), f'Ground truth has {len(labels_list)} frames, BUT got {len(anomaly_score_total_list)} detected frames!'
    auc = AUC(anomaly_score_total_list, np.expand_dims(1 - labels_list, 0))
    ...
    # score_list: is a list of anomaly scores of each video
    return auc, fpr, tpr, thresholds, score_list
