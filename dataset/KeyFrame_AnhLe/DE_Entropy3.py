...
def getEntropy(source, numKeyFrames, KF):
    ...
    for i in range(0, numKeyFrames - 1):
        while True:
            try:
                im1 = cv2.imread(os.path.join(source, '{:03d}.jpg'.format(KF[i])), 0)
                im2 = cv2.imread(os.path.join(source, '{:03d}.jpg'.format(KF[i + 1])), 0)
...
def evolution(source, numGeneration, popSize, numFrames, numKeyFrames, keyframe_dir, path2file):
    """
    ...
    for frame_number in best_parent[:-1]:
        keyframe = os.path.join(source, '{:03d}.jpg'.format(frame_number))
