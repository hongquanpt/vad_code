import argparse
from system.ConvVRNN.default import update_config, cfg # for ConvVRNN
...
from model.ConvVRNN.ConvVRNNModel import ConvVRNNModel
...
def train(cfg, device, logger, writer, checkpoint_dirpath):
    ...
    #====================================
    ConvVRNN = ConvVRNNModel(cfg, device, logger, writer)
    ConvVRNN.train(train_dataloader, checkpoint_dirpath)
...
def test(cfg, device, logger, writer, visualization_dirpath):
    ...
    for i in range(0, len(cfg.TEST.checkpoint_filepaths)):
        ...
        ConvVRNN.test(test_dataloader, label_list, checkpoint_filepath, sub_visualization_dirpath)
    return 0
...
def parse_args():
    ...
    cfg_filename = 'system/ConvVRNN/ped2.yaml'
