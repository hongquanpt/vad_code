...
# step 1: chmod u+x system/ConvAE/train.sh
# step 2: ./system/ConvAE/train.sh
...
config_filepath='system/ConvAE/ped2.yaml' # avenue.yaml, shanghaitech.yaml
#loop=(1 2 3 4 5)
loop=(1)
...
for id in "${loop[@]}"; do
    echo ">>> main_ConvAE.py >>>>"
    python main_ConvAE.py --cfgfile "$config_filepath" SYSTEM.phase 'train' SYSTEM.loop_index "$id"
done
conda deactivate