...
def getEntropy_Shanghai(source, numFrames):
    entropy_sum = 0
    global En
    En[:]=[]
    global SubEn
...
def extract_keyframes(source, dest):
    ...
    for i in range(len(videos)):
        ...
        deletefiles_in_dir(keyframe_dir)
        ...
        for frame_number in range(len(SubEn)):
            if SubEn[frame_number] >= medium:
                ...
                keyframe = os.path.join(sourceEvo, '{:04d}.jpg'.format(frame_number+2))
                images_for_gif.append(imageio.v2.imread(keyframe))
                file_name = keyframe.split('\\')[-1]
                dest_file = os.path.join(keyframe_dir, file_name)
                shutil.copy(keyframe, dest_file)
                fieldnames = ['id', 'name']
                row = {'id': str(frame_number+2), 'name': file_name}
                writefile_csv_dic(path2file, 'a', fieldnames, row)
...
def main_extract_keyframes():
    ...
    parser.add_argument('--source', type=str, default='.\\dataset\\shanghai_tiny\\training', help='source file')
    parser.add_argument('--dest', type=str, default='.\\dataset\\shanghai_tiny', help='destination folder')
