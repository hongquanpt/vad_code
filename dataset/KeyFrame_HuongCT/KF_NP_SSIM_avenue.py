...
# Calculate Average Entropy Difference for a chromosome.
def getSSIM4Num(source, numFrames):
    distance_sum = 0
    global SubEn
    SubEn[:] = []
    for i in range(0, numFrames - 1):
        distance = getSSIM2Image4Num(source, i, i + 1)
        distance_sum += distance
        SubEn.append(distance)
...
def extract_keyframes(source, dest):
    ...
    for i in range(len(videos)):
        ...
        if video_name in {'05', '11', '12', '13', '14', '15', '16'}:
            ...
            starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
            starter.record()
            getSSIM4Num(sourceEvo, numFrames)
            print('Medium Co=', medium)
            ...
            while (frame_start < (numFrames - 1)) and (frame_end < numFrames):
                if getSSIM2Image4Num(sourceEvo, frame_start, frame_end) <= medium:
                    # print('KC=', getAED2Image(sourceEvo, frame_start, frame_end))
...
def main_extract_keyframes():
    ...
    source_dir = os.path.join('dataset', 'avenue_tiny', 'training')
    print('Source=', source_dir)
    dest_dir = os.path.join('dataset', 'avenue_tiny')
