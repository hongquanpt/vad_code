...
# Khac voi memory_module_MNAD.py
# Boi vi them cac ham: get_score_Euclidean, get_score_cosine, get_score_Mahalanobis
device = "cuda:0"
...
class Memory(nn.Module):
    def __init__(self, memory_size, feature_dim, key_dim,
                 temp_update, temp_gather, memory_device="cuda:0"
                 ):
    ...
    def get_update_query(self, mem, max_indices, update_indices, score, query, train):
        # print('============>>>>> memory_module: device = ', device)
        m, d = mem.size()
        if train:
    ...
    def get_score_Euclidean(self, mem, query):
        bs, h, w, d = query.size()  # 4, 32, 32, 512
        m, d = mem.size()  # 10, 512

        query = query.reshape(bs * h * w, d)
        a = torch.randn(bs * h * w, 1)
        list_scores = []
        # get each item (element) in memory
        i = 0

        for i, item in enumerate(mem):
            # print("i = ", i)
            # https://pytorch.org/docs/stable/generated/torch.sum.html
            score = (query - item).pow(2).sum(dim=1).sqrt()
            # https://stackoverflow.com/questions/72463818/concatenate-n-pytorch-tensors-of-the-same-shape-generated-from-within-loop
            list_scores.append(score)

        # tensor_scores = torch.tensor(list_scores)
        # https://stackoverflow.com/questions/69115837/how-to-concatenate-a-list-of-tensors-on-a-specific-axis
        # score = torch.cat(tensor_scores, axis = 1)

        # https://discuss.pytorch.org/t/how-to-turn-a-list-of-tensor-to-tensor/8868/11
        score = torch.stack(list_scores, dim=1)
        # print("score.size() = ", score.size()) # (4096, 10), here 4096 = bsxhxw = 4x32x32
    ...
    def get_score_cosine(self, mem, query):
        bs, h, w, d = query.size()
        m, d = mem.size()
    ...
    def get_score_Mahalanobis(self, mem, query):
        bs, h, w, d = query.size()
        m, d = mem.size()
    ...
    def get_score(self, mem, query):
        score_query, score_memory = self.get_score_Euclidean(mem, query)
        return score_query, score_memory

    def forward(self, query, keys, train=True, pseudo_anomaly=False):
        ...
        if train:
            ...
            # update
            # pseudo_anomaly = False neu khong update thi ket qua xau di
            if pseudo_anomaly:  # khong update memory!
                updated_memory = keys
            else:  # update memory
