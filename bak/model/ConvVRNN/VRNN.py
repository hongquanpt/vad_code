...
class VRNN(nn.Module):
    ...
    def forward(self,x,seq_len=12, step=3):
        #h = Variable(torch.zeros(x[0].size()).cuda())
        x_0 = x[:, 0:3, :, :]
        #print(f'x_0.size() = {x_0.size()}') #torch.Size([1, 3, 256, 256])
        h = Variable(torch.zeros(x_0.size()).cuda())
        #print("shape of h:{}".format(h.size())) #torch.Size([1, 3, 256, 256])
        # hidden_state
        ...
        for t in range(0, seq_len, step):
            ...
            enc_t = self.encoder(torch.cat((x_t, h),1))
            ...
            recon_loss += self.recons_loss(dec_t, x_t)
