...
def train(cfg, device, logger, writer, checkpoint_dir_path):
    ...
    # ====================================
    astnet = ASTNetModel(cfg, device, logger, writer)
    ...
    astnet.train(train_dataloader, checkpoint_filepath, checkpoint_dir_path)
    return 1
...
def test(cfg, device, logger, writer, visualization_dir_path):
    ...
    astnet.test(test_dataloader, label_list, checkpoint_filepath, sub_visualization_dir_path)
    return 0
...
if __name__ == '__main__':
    """
    python main_ASTNet.py --phase 'train' --dataset 'ped2' SYSTEM.device "cuda:0"
    """
    main(net='ASTNet')
